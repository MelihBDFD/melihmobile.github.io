...
const todoView = () => {
  ...
  const addTodo = (todo, isNew = false, sort) => {
    ...
    if (totalSubtasks) {
      ...
      if (remainingSubTasks === 1) {
        subtaskIndicatorLabel.dataset.tooltip = 'Tamamlanacak bir alt görev kaldı';
        subtaskIndicator.dataset.tooltip = '';
      } else if (remainingSubTasks === 0) {
        subtaskIndicator.dataset.tooltip = 'Tüm alt görevler tamamlandı';
      } else {
        subtaskIndicatorLabel.dataset.tooltip = `
        ${remainingSubTasks} tamamlanacak alt görev
        `;
        subtaskIndicator.dataset.tooltip = '';
      }
    }
  ...
  const displaySearchResults = (todos) => {
    ...
    if (elements.tasksView.dataset.projectIndex) {
      ...
      emptyStateText.textContent = "Üzgünüz, aradığınızı bulamadık.";
  ...
  const displayDetails = (todo, currentProject, sort, saveData) => {
    ...
    if (todo.date) {
      ...
      dateMessage.innerHTML = 'Son tarih ekle';
    }
    ...
    priorityLow.dataset.tooltip = 'Düşük';
    priorityMedium.dataset.tooltip = 'Orta';
    priorityHigh.dataset.tooltip = 'Yüksek';
    ...
    const handleSubmit = (e) => {
      ...
      const remainingSubTasks = totalSubtasks - completedSubtasks;
      if (remainingSubTasks === 1) {
        liveSubtaskIndicatorLabel.dataset.tooltip = 'Tamamlanacak bir alt görev kaldı';
      } else {
